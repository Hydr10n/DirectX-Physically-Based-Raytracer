...
private:
	...
	void LoadScene(const path& filePath) {
		...
		try {
			...
			m_scene->Load(*sceneDesc, *m_resourceDescriptorHeap, descriptorHeapIndex);
				...
				if (ImGui::TreeNodeEx("Post-Processing", ImGuiTreeNodeFlags_DefaultOpen)) {
					...
					if (ImGui::TreeNodeEx("Tone Mapping", ImGuiTreeNodeFlags_DefaultOpen)) {
						...
						if (m_deviceResources->IsHDREnabled()) {
							ImGui::SliderFloat("Paper White Nits", &toneMappingSettings.PaperWhiteNits, toneMappingSettings.MinPaperWhiteNits, toneMappingSettings.MaxPaperWhiteNits, "%.2f", ImGuiSliderFlags_AlwaysClamp);
