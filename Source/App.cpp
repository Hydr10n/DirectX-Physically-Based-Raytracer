...
struct App::Impl : IDeviceNotify {
	Impl(WindowModeHelper& windowModeHelper) noexcept(false) : m_windowModeHelper(windowModeHelper) {
		...
		if (const auto filePath = ResolveResourcePath(L"Assets/Scenes/Default.json"); exists(filePath)) {
			LoadScene(filePath);
		}
	}
...
private:
	...
	void LoadScene(const path& filePath) {
		...
		try {
			sceneDesc = make_unique<MySceneDesc>(filePath);
	...
	void RenderUI() {
		...
		if (m_UIStates.IsFileDialogOpen) {
			RenderFileDialog();
		}
		...
		if (!empty(m_sceneErrorMessage) || IsSceneLoading()) {
	...
	void RenderFileDialog() {
		...
		if (const auto workSize = ImGui::GetMainViewport()->WorkSize;
			instance.Display(Key, ImGuiWindowFlags_NoCollapse, { workSize.x / 2, workSize.y / 2 })) {
			if (instance.IsOk()) {
				m_futures["LoadScene"] = async(
					launch::deferred,
					[&] { LoadScene(instance.GetFilePathName()); }
				);
			}
