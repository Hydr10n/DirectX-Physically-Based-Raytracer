...
export {
	namespace DirectX::SimpleMath {
		...
		FROM_JSON_FUNCTION_PROTOTYPE(Quaternion) {
			...
			if (nlohmann_json_t.x == 0 && nlohmann_json_t.y == 0 && nlohmann_json_t.z == 0) from_json(nlohmann_json_j, reinterpret_cast<Vector4&>(nlohmann_json_t));
			else nlohmann_json_t = Quaternion::CreateFromYawPitchRoll(XMConvertToRadians(nlohmann_json_t.y), XMConvertToRadians(-nlohmann_json_t.x), XMConvertToRadians(-nlohmann_json_t.z));
		}
