...
inline HitInfo GetHitInfo(uint instanceIndex, float3 worldRayOrigin, float3 worldRayDirection, float rayT, float4x3 objectToWorld, uint primitiveIndex, float2 barycentrics) {
	...
	const StructuredBuffer<VertexPositionNormalTextureTangent> vertices = ResourceDescriptorHeap[localResourceDescriptorHeapIndices.TriangleMesh.Vertices];
	...
	if (localResourceDescriptorHeapIndices.Textures.NormalMap != ~0u) {
		...
		float3 T;
		if (g_localData[instanceIndex].HasPerVertexTangents) {
			const float3 tangents[] = { vertices[indices[0]].Tangent, vertices[indices[1]].Tangent, vertices[indices[2]].Tangent };
			T = normalize(Vertex::Interpolate(tangents, barycentrics));
		}
		else {
